...
# Script from https://github.com/pennekampster/intrinsic_predictability_eco/blob/v1.0.0/Code/pe_code.R
...
# compare PE and WPE estimates --------------------------------------------

library(tidyverse)
library(statcomp)

data_test1 <- readRDS("data_test1.rds")
data_test2 <- readRDS("data_test2.rds")

id1 <- data_test1$ID %>% unique()
id2 <- data_test2$ID %>% unique()

for (i in 1:length(id1)) {

  x1 <- filter(data_test1, ID == id1[i]) %>%
    select(contains("Sim.")) %>%
    as.matrix()

  x1_opd <- apply(x1, 2, function(x) ordinal_pattern_distribution(x, ndemb = 3))

  for (h in 1:100) {
    test_that(paste("correct PE calculation, test1", i, h, sep = "_"), {
      expect_equal(round(calculate_pe(x1[,h])[2][[1]], 2), round(permutation_entropy(x1_opd[,h]), 2))
    })

    test_that(paste("correct WPE calculation, test1", i, h, sep = "_"), {
      expect_equal(round(calculate_pe(x1[,h])[1][[1]], 2),
                   round(PE(x1[,h], weighted = T, word_length = 3, tau = 1, tie_method = "first"), 2))
    })
  }
}
...
for (i in 1:length(id2)) {
  ...
  x2 <- filter(data_test2, ID == id2[i]) %>%
    select(contains("Sim.")) %>%
    as.matrix()

  x2_opd <- apply(x2, 2, function(x) ordinal_pattern_distribution(x, ndemb = 3))

  for (h in 1:100) {
    test_that(paste("correct PE calculation, test1", i, h, sep = "_"), {
      expect_equal(round(calculate_pe(x2[,h])[2][[1]], 2), round(permutation_entropy(x2_opd[,h]), 2))
    })

    test_that(paste("correct WPE calculation, test1", i, h, sep = "_"), {
      expect_equal(round(calculate_pe(x2[,h])[1][[1]], 2),
                   round(PE(x2[,h], weighted = T, word_length = 3, tau = 1, tie_method = "first"), 2))
    })
  }
}
